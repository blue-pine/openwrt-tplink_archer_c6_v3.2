...
dlogin_iface_event()
{
    ...
    if [ -n "$alexa_smart_home_support" -a "$alexa_smart_home_support" = "yes" ]; then
        insmod $DLOGIN_KER_PATH/domain_dns.ko lan_ip=$_ipaddr dut_domain="\"$_domains\"" ignore_me=0 not_router=$not_router discover_domain="\"_alexa._tcp.local router._alexa._tcp.local\""
    else
        insmod $DLOGIN_KER_PATH/domain_dns.ko lan_ip=$_ipaddr dut_domain="\"$_domains\"" ignore_me=0 not_router=$not_router
    fi
    ...
    [ "$mode" != "ap" ] && dlogin_dnsmasq_restart
