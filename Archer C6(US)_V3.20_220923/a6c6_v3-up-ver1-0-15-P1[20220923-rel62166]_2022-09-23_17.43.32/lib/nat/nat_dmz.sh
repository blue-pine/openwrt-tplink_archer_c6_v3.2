...
nat_load_dmz() {
	...
	[ "$wanproto" = "dslite" -o "$wanproto" = "v6plus" ] && {
		echo "dmz not support dial type  dslite or v6plus now"
		return 0
	}
    [ "$nat_dmz_enable" == "on" ] && {
        nat_syslog 23
        ...
        # We have to flush all conntrack, for pptp
        echo f >/proc/net/nf_conntrack 2>/dev/null
        nat_syslog 53 "$nat_dmz_ipaddr"
    }

    [ "$nat_dmz_enable" == "off" ] && {
		# fix bug 477403, flush conntrack of the reply ip when disable dmz
		conntrack -D -r "$nat_dmz_ipaddr" 2>/dev/null
		nat_syslog 24
	}
}
