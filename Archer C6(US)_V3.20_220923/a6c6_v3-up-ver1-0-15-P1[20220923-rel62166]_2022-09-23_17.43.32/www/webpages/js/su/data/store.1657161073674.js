...
$.su.Store = function(options){
	var defaults = {
		...
		//为了兼容增、删、改三种条目操作的数据格式，将updateMode细分为updateMode、insertMode、removeMode
	...
	var operationMode = {
		//"operation" === store.updateData|store.insertData|store.removeDataByKey|store.removeDataByIndex
		//"complete" === store.loadData
		//"reload" === store.load

		//由于此前框架updateMode显示指定complete代表三种操作模式下均为complete，
		//此处采用向前兼容方式采取优先级赋值，除非显示指定insertMode|removeMode，否则初始化insertMode|removeMode为updateMode
		//未显示配置insertMode|removeMode且未配置updateMode模式下，insertMode|removeMode初始化为"operation"
		//使用过程中须注意，一旦配置updateMode为非"operation"模式，如果insertMode|removeMode != updateMode, 需要显示指定。
		updateMode: options.updateMode || "operation",
		insertMode: options.insertMode || options.updateMode || "operation",
		removeMode: options.removeMode || options.updateMode || "operation"
	}

	var settings = $.extend({}, defaults, options, operationMode);
...
$.su.Store.prototype.insert = function(index, data, _callback, _callback_failed, _callback_error){
	...
	this.proxy.write($.extend(opt, {
		...
		if(store.insertMode == "operation"){
			//operation模式下要求返回条目信息，包含store fields中定义的所有字段
			...
			//reload模式下不关心返回数据格式，重新刷新表格获取表格所有数据
			store.load({}, function(data, others, status, xhr){
				if (_callback){
					_callback.call(store, data, others, status, xhr);
				};
			});
...
$.su.Store.prototype.updateColumns = function(columns, data, _callback, _callback_failed, _callback_error){
	var store = this,
		data = data || {};
	...
	this.proxy.read($.extend({
		...
		for(var i=0; i<store.data.length; i++){
			for(var j=0; j<data.length; j++){
				if(store.data[i].key == data[j].key){
					for(var m=0; m<columns.length; m++){
						store.data[i][columns[m]] = data[j][columns[m]];
					}
				}
			}
		}
		for(var i=0; i<store.snapshot.length; i++){
			for(var j=0; j<data.length; j++){
				if(store.snapshot[i].key == data[j].key){
					for(var m=0; m<columns.length; m++){
						store.snapshot[i][columns[m]] = data[j][columns[m]];
					}
				}
			}
		}
		...
		$(store).trigger("ev_updatecolumns", [columns, data]);
	}, function(error, others){
		if (_callback_failed){
			_callback_failed.call(store, error, others);
		};
		$(store).trigger("ev_failed", ["updatecolumns", error, others]);
	}, function(xhr, status, type) {
		if (_callback_error){
			_callback_error.call(store, xhr, status, type);
		};
		$(store).trigger("ev_error", ["updatecolumns", xhr, status, type]);
	});
};
