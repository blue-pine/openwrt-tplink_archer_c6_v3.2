...
$.su.Widget("checkbox", {
		...
		me.each(function(i, obj){
			...
			for (var index = 0; index < items.length; index++){
				...
				var funcInitLi = function(boxName, boxValue, boxId, boxlabel){
					...
					inHTML += 					"</div>";
					inHTML += 				"</li>";
					inHTML += 				"<div class=\"checkbox-item-tips\">";
					inHTML += 					"<span></span>";
					inHTML += 				"</div>";
				};
	...
	setValue: function(me, _value){
		...
		for (var index = 0; index < value.length; index++){
			var checkBox = container.find("input[value="+value[index]+"][type=checkbox]");
			if (checkBox.length){
			...
			var hiddenBox = container.find("input[value="+value[index]+"][type=hidden]");
			if (hiddenBox.length){
	...
	setItemTips: function(me, params){
		var me = me || this,
			obj = me.get(0),
			container = me.checkbox("getContainer"),
			checkboxs = container.find("input.checkbox-checkbox"),
		...
		checkboxs.each(function(i, obj){
			var tar = $(obj);
			if (tar.val() in valueObj){
				tar.closest("ul.checkbox-group-list-wrap").find("div.checkbox-item-tips").find("span").html(params[2]);
			};
		});
