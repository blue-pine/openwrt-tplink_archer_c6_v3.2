	...
	create: function(defaults, options){
		...
		container.delegate("div.combobox-wrap", "click", function(e){
			...
			if (toggleFlag === "hidden"){
				...
				$(window).trigger("ev_comboWrapShow");
			}else{
			...
			container.children("div.widget-error-tips.show").css("display", "none");
...
//			e.stopPropagation();
	...
	hideItem: function(me, valueArray){	//å®žçŽ°å¯¹å•ä¸ªå¯¹è±¡çš„å¯ç”¨ç¦ç”¨
		var me = me || this,
			container = me.combobox("getContainer"),
			checkboxs = container.find("input.combobox-checkbox"),
			valueArray = valueArray[1];
		...
		checkboxs.each(function(i, obj){
			var tar = $(obj);
			if (tar.val() in valueObj){
				tar.closest("li.combobox-list").hide();
				tar.attr("disabled", true);
			};
		});
	...
	showItem: function(me, valueArray){	//å®žçŽ°å¯¹å•ä¸ªå¯¹è±¡çš„å¯ç”¨ç¦ç”¨
		var me = me || this,
			container = me.combobox("getContainer"),
			checkboxs = container.find("input.combobox-checkbox"),
			valueArray = valueArray[1];
		...
		checkboxs.each(function(i, obj){
			var tar = $(obj);
			if (tar.val() in valueObj){
				tar.closest("li.combobox-list").show();
				tar.attr("disabled", false);
			};
		});
	...
	selectNoTrigger: function(me, valueArray){
       ...
       setLabel: function(me, _label){
               ...
               var label = container.find("label.combobox-fieldlabel");
               if (label.get(0))
               {
                       label.html(labelTxt);
               }
