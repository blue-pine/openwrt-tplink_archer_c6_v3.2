...
$.su.Widget("form", {
	...
	create: function(defaults, options){
				...
				if (type === "string"){
					if (obj.submitBtn === "default"){
						submitBtn = $("<button type=\"button\"></button>").button({
							...
							cls: "submit",
							handler: function(e){
								me.triggerHandler("beforeSubmit");
		...
		proxy.on("ev_read", function(e, result, status, xhr){
			...
			//作相应的差错处理...
			me.form("hideLoading");
			me.triggerHandler("ev_failed", [me, errorcode, others, data]);
		});
	...
	validate: function(me){
		...
		me.triggerHandler("ev_validatechange", [result, fg]);
		return result;
	},
	doSubmit: function(me, params){
		...
		if (traditional){
			...
			iframe.one("load", function(){
				var doc;
				try{
					doc = document.getElementById(form_id).contentDocument.body.textContent || document.getElementById(form_id).contentDocument.body.innerHTML;
				}
				catch(error){
					//some error
				}
				if(proxy){
					...
					if(typeof(data) == "object"){
						if(data.success){
							proxy.callbackSuccess(data, callback);
						}
						else{
							proxy.callbackFail(data, callback_failed);
						}
					}
					else{
						proxy.callbackError(callback_error);
					}
				}
			});
			return;
	...
	loadData: function(me, params){	//data	//非ajax操作，从本地加载数据，并且修改form中的数据存储
		...
		me.triggerHandler("ev_beforeLoadData", data);
		...
		me.triggerHandler("ev_loadData", data);
	},
	...
	serialize: function(me){
		...
		var disabled = me.get(0).sendDisableData? form.find(':input:disabled').removeAttr('disabled') :$();
