...
(function($){
	...
	$.su.Widget("password", {
		...
		create: function(defaults, options){
			...
			me.each(function(i, obj){
				...
				if(obj.showCheckTips){
					inHTML +=			"<div class=\"password-check-tips "+obj.inputCls+" hidden\">";
					inHTML +=				"<div class=\"password-check-tips-wrap\">";
					inHTML +=					"<div class=\"check-tips check-tips-1\">" + $.su.CHAR.OPERATION.PASSWORD_CHECK_TIPS1 + "</div>";
					inHTML +=					"<div class=\"check-tips check-tips-2\">" + $.su.CHAR.OPERATION.PASSWORD_CHECK_TIPS2 + "</div>";
					inHTML +=					"<div class=\"check-tips check-tips-3\">" + $.su.CHAR.OPERATION.PASSWORD_CHECK_TIPS3 + "</div>";
					inHTML +=				"</div>";
					inHTML +=			"</div>";
				}
			...
			container.delegate("input.password-text", "blur", function(e){
				...
				container.find(".password-check-tips").addClass("hidden");
		...
		inputCheck: function(me, arg){
			...
			if(value.indexOf(" ") == -1){
				container.find(".check-tips-1").addClass("correct");
				if(arg[1]){
					container.find(".check-tips-1").addClass("hidden");
				}
			}else{
				container.find(".check-tips-1").removeClass("correct");
				checked = false;
			}
	
			// Must be 6-32 characters long.
			if(value.length >= 6 && value.length <= 32){
				container.find(".check-tips-2").addClass("correct");
				if(arg[1]){
					container.find(".check-tips-2").addClass("hidden");
				}
			}else{
				container.find(".check-tips-2").removeClass("correct");
				checked = false;
			}
			...
			var regSymbol = /[\x21-\x2f\x3a-\x40\x5b-\x60\x7b-\x7e]+/
			var tag = 0;
			...
			if(tag >= 2){
				container.find(".check-tips-3").addClass("correct");
				if(arg[1]){
					container.find(".check-tips-3").addClass("hidden");
				}
			}else{
				container.find(".check-tips-3").removeClass("correct");
				checked = false;
			}
	
			if(arg[1] && !checked){
				container.find(".password-check-tips").removeClass("hidden");
				container.find(".password-check-tips").height(container.find(".password-check-tips-wrap").outerHeight());
				container.find(".password-check-tips").addClass("error");
				$(this).textbox("setError", "");
			}
	
			return checked;
		},
		clearInputCheck: function(){
			var me = me || this,
				obj  = me.get(0),
				container = me.closest("div.password-container");
			
			container.find(".check-tips-1").removeClass("hidden");
			container.find(".check-tips-2").removeClass("hidden");
			container.find(".check-tips-3").removeClass("hidden");
			container.find(".password-check-tips").removeClass("hidden");
			container.find(".password-check-tips").height(container.find(".password-check-tips-wrap").outerHeight());
			container.find(".password-check-tips").removeClass("error");
		},
	...
	/*			password = container.find("input.password-text"),
			...
			if(obj.showCheckTips && !me.password("inputCheck", true)){
				return false;
			}
		...
		getValue: function(me){
			var me = me || this,
				container = me.closest("div.password-container");
			if(me.get(0).passwordVisible){
				var password = container.find("input.password-text.hidden");
			}else{
