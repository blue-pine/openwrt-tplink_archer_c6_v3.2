...
$.su.Widget("radio", {
	...
	loadData: function(me, items){
		...
		var funcInitLi = function(boxName, boxValue, boxId, boxlabel, boxCls){
			...
			inHTML +=						"<label class=\"radio-label "+boxCls+" "+checkedCls+"\" for=\""+boxId+"\">";
																								//boxlabel contains ' or " will cause UI bug
			inHTML += 							"<input class=\"radio-radio\" type=\"radio\" display=\""+$.su.func.escapeHtml(boxlabel)+"\" name=\""+boxName+"\" value=\""+boxValue.toString()+"\" id=\""+boxId+"\" "+checked+" />";
	...
	getSelectedText: function(me){
		...
		return $.su.func.unEscapeHtml(radioBox.attr("display"));
	},
