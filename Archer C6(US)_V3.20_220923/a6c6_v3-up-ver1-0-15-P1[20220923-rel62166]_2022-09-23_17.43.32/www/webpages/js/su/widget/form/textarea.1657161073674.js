	...
	create: function(defaults, options){
		...
		me.each(function(i, obj){
			...
			if (this.fieldLabel !== null){
				...
				if (this.hint){
					inHTML +=			"<span class=\"hint text-hint\">" + this.hint+ "</span>";
				};
		...
		container.delegate("textarea.textarea-text", "focus", function(e){
			container.find("span.text-hint").fadeOut(50);
			$(this).textarea("setFocus");
		}).delegate("textarea.textarea-text", "keyup blur", function(e){
			var t = $(this);
			if(!t.val() && e.type == "focusout"){
				container.find("span.text-hint").fadeIn(50);
			}
			if (document.all){
		...
		}).delegate("span.text-hint", "focus click", function(e){
			e.preventDefault();
			if(container.find("span.text-text").attr("readonly") !== "readonly"){
