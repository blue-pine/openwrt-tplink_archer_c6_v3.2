...
$.su.Widget("textbox", {
	...
	create: function(defaults, options){
		...
		me.each(function(i, obj){
			...
			//自定义监听事件
			if(this.checkFocusTarget){
				var container = me.closest("div.widget-container");
				container.delegate("input.text-text", "blur", function(e){
					var selfFunction = this.checkFocusTarget;
					setTimeout(function(){
						//true表示输入框失焦来源不需要输入框做验证
						if(selfFunction(e)){
							clearTimeout(validate_timeout_id);
						}
					},10);
				});
			}
		...
		container.delegate("input.text-text", "click", function(e){
			...
			if($(this).attr("readonly") !== "readonly"){
				var textboxObj = $(this);
				validate_timeout_id = setTimeout(function(){
					textboxObj.textbox("validate");
				},20);
			}
			...
			e.preventDefault();
			if(container.find("input.text-text").attr("readonly") !== "readonly"){
	...
	setValue: function(me, _value){
		var me = me || this,
			obj = me.get(0),
			value = _value[1],
			hint = me.closest("div.text-container").find("input.text-hint");
	...
	setAttribute: function(me, args){
		var me = me || this;
		var attr = args[1];
		var attrValue = args[2];
		if(me[0][attr] !==undefined){
			me[0][attr] = attrValue
		}
	},
	getContainer: function(me){
        ...
        setLabel: function(me, _label){
               var me = me || this,
                       container = me.textbox("getContainer");
                       var labelTxt = _label[1];
               var label = container.find("label.text-fieldlabel");
               if (label.get(0))
               {
                       label.html(labelTxt);
               }
