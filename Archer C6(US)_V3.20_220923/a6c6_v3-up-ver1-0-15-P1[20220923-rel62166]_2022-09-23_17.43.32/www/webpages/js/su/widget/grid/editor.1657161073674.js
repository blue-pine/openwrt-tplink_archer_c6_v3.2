	...
	create: function(defaults, options){
		...
		me.each(function(i, obj){
			...
			var inHTML = 	"<td class=\"editor-container\" colspan=\""+columns.length+"\">";
				...
				inHTML +=					"<span class=\"button-button-before\"></span>";
				inHTML +=					"<span class=\"button-text text\">"+$.su.CHAR.OPERATION.SAVE+"</span>";
				inHTML +=				"</button>";
				inHTML +=			"</div>";
		...
		if (form.form("validate")){
			...
			if (editingId != "add"){
				...
				store.update(editingId, {
					...
					if(obj.successCallback) obj.successCallback(arguments);
					me.editor("cancelEdit");
				}, function(){
					grid.grid("prompt", false);
					if(obj.errorCallback) obj.errorCallback(arguments);
				}, function(){
					grid.grid("prompt", false);
					if(obj.failCallback) obj.failCallback(arguments);
				});
			}else{
				//新增
				store.insert(len, {
			...
			//console.log(editingId, dOld, dNew);
