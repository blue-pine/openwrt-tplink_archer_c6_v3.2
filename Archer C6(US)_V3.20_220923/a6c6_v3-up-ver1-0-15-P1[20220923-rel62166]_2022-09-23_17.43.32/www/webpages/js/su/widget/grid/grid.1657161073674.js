...
$.su.Widget("grid", {
	...
	create: function(defaults, options){
		...
		me.each(function(i, obj){
			...
			var lineHeight = grid.find("tr.grid-content-tr").outerHeight() + 1;
			...
			if (obj.maxLines){
				styleText +=	"max-height:" + (obj.maxLines - 1 + obj.maxLines*lineHeight) + "px;";
		...
		if(store.updateMode == "operation"){
			$(store).on("ev_updatedata", function(e, key, index, data){
				me.grid("update", key, index, data);
			})
		}
		if(store.updateMode == "operation"){
			$(store).on("ev_updatecolumns", function(e, columns, data){
				me.grid("updateColumns", columns, data);
			});
		}
		if(store.removeMode == "operation"){
			$(store).on("ev_removedata", function(e, keyArray, indexArray){
				// if (paging && paging.isPaging){
				// 	$(paging).paging("loadPage", paging.currentPage);
				// }else{
		...
		me.delegate("a.grid-content-btn.grid-content-btn-status", "click", function(e){
			...
			var	beforeDelete = me.get(0).beforeDelete,
				key = btn.attr("data-key"),
				editorObj = me.grid("getEditor");

			if (editorObj && editorObj.isEditor && editorObj.editing){
				$(editorObj).editor("shake");
				return;
			};
			if (typeof beforeDelete == "function" && !beforeDelete([key], "row")) {
				return;
			}
	...
	initRow: function(me, params){	//row-index, data 返回html值
		...
		for (var kndex = 0; kndex < len; kndex++){
			var column = columns[kndex],
				dd = (data[column.dataIndex] == undefined) ? "---" : data[column.dataIndex],
				fst = (kndex === 0) ? "fst" : "",
				lst = (kndex === len-1) ? "lst" : "";
			//console.log(column.xtype, column.dataIndex, data[column.dataIndex], column.renderer);
			switch (column.xtype){
				case "rownumberer":
					inHTML += 	"<td class=\"grid-content-td grid-content-td-"+kndex+" grid-content-td-row-numberer "+fst+" "+lst+"\" name=\"row-numberer\">";
						...
						okHandler: function(e){
							...
							if (selectedKeys.length > 0){
								...
								store.remove(selectedKeys, {}, function(){
									me.grid("prompt", true);
									// if(me.find(".grid-header-checkbox").hasClass("selected")){
									// 	me.find(".grid-header-checkbox").removeClass("selected");
									// 	me.find(".grid-header-checkbox").find(".checkbox-label").removeClass("checked");
									// }
								}, function(){
									me.grid("prompt", false);
								}, function(){
			...
			toolbar.delegate("a.btn-add", "click", function(e){
				...
				if (isEditing === true){
					...
					if (selectedKeys.length == 0){
						...
						if (typeof beforeDelete == "function" && !beforeDelete(selectedKeys)) {
							return;
						}
	...
	updateColumns: function(me, params){
		var me = me || this,
			obj = me.get(0),
			rows = obj.rows,
			columns = params[1],
			data = params[2],
			initColumns = me.grid("getColumns"),
			inHTML = "";

		for(var i=0; i<rows.length; i++){
			for(var j=0; j<data.length; j++){
				if(rows[i].attr("data-key") == data[j].key){
					for(var m=0; m<columns.length; m++){
						for(var n=0; n<initColumns.length; n++){
							if(columns[m]==initColumns[n].name){
								inHTML = initColumns[n].renderer(data[j][columns[m]], 0, data[j]);
								me.find("tr.grid-content-tr-"+rows[i].attr("data-key")+" td.grid-content-td-"+columns[m]+" div.content").html(inHTML);
								break;
							}
						}
					}
